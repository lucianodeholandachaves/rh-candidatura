...
# ─────────────────────────────────────────────────────────
# PERSISTÊNCIA DA API KEY — salva/carrega de arquivo local
# ─────────────────────────────────────────────────────────
...
def _excel(b):
    try:
        ...
        s, df = dfs[0]
        # Converte nomes de coluna para string (evita erro com datas no cabecalho)
...
def consolidar_arquivos(lista_dfs: List[pd.DataFrame]) -> pd.DataFrame:
    ...
    def get_periodo_col(df, nomes):
        return next((c for c in df.columns if str(c).lower().strip() in nomes), None)
...
for k,v in {
    "passo":1, "cid":None,
    "api_key": _config.get("anthropic_api_key",""),  # carrega do arquivo
    "arquivos_carregados":[],
    "df_consolidado":None,
    "sug_ia":None, "mapa":{},
    "df_final":None, "log":[]
...
# API KEY — sempre visível, nunca some
with st.container():
    st.markdown('<div class="sec">🔑 Anthropic API Key (Claude)</div>', unsafe_allow_html=True)
    col_ak1, col_ak2, col_ak3 = st.columns([3, 1.5, 1])
    with col_ak1:
        ak = st.text_input(
            "Cole sua API Key aqui (sk-ant-...)",
    ...
    with col_ak2:
        if st.session_state.api_key:
            st.markdown('<div class="box-ok" style="margin-top:28px">🟢 IA ativa — salva</div>', unsafe_allow_html=True)
        else:
...
# ═════════════════════════════════════════════════════════
    ...
    # ── ARQUIVO (MÚLTIPLOS — UM POR VEZ) ────────────────
    if tipo == "arquivo":

        st.markdown("""
        <div class="box-info">
        📂 Adicione os arquivos <b>um por vez</b> clicando em <b>+ Adicionar arquivo</b>.<br>
        Pode adicionar a DRE, depois o Balanço, depois o Fluxo de Caixa — cada um separado.<br>
        O sistema acumula tudo e consolida automaticamente.
        </div>""", unsafe_allow_html=True)

        # Upload de UM arquivo por vez com chave que muda para resetar o widget
        ...
        # Botão ADICIONAR — lê o arquivo e empilha na lista
        if novo_arquivo is not None:
            ...
            if novo_arquivo.name not in nomes_ja:
                ...
                with col_add:
                    if st.button("➕ Adicionar este arquivo", use_container_width=True, key="btn_add_arq"):
                        ...
                        if df_novo is not None:
                            st.session_state.arquivos_carregados.append({
                                ...
                                "linhas": len(df_novo), "colunas": df_novo.shape[1],
                                "ok": True, "msg": msg_r
                            })
                            log(f"'{novo_arquivo.name}' adicionado: {msg_r}")
                        else:
                            st.session_state.arquivos_carregados.append({
                            ...
                            log(f"Erro em '{novo_arquivo.name}': {msg_r}", "err")
                        # Incrementa o counter para resetar o file_uploader
                        st.session_state.upload_counter += 1
                        st.rerun()
                with col_info:
                    st.markdown(f'<div class="box-info">📄 <b>{novo_arquivo.name}</b> pronto para adicionar</div>',
                                unsafe_allow_html=True)
            else:
                st.markdown(f'<div class="box-warn">⚠️ <b>{novo_arquivo.name}</b> já está na lista abaixo.</div>',
                            unsafe_allow_html=True)
        ...
        if st.session_state.arquivos_carregados:
            ...
            for i, arq in enumerate(st.session_state.arquivos_carregados):
                ...
                with c4:
                    if arq["ok"]:
                        st.markdown('<span style="color:#2EC4B6;font-weight:600">Lido ✓</span>', unsafe_allow_html=True)
                    else:
                        st.markdown(f'<span style="color:#E63946;font-weight:600" title="{arq["msg"]}">Erro ✗</span>', unsafe_allow_html=True)
                with c5:
                    if st.button("🗑", key=f"rem_{i}", help=f"Remover {arq['nome']}"):
                        st.session_state.arquivos_carregados.pop(i)
                        st.session_state.df_consolidado = None
                        st.rerun()

                # Preview individual — botão toggle abre/fecha
                if arq["ok"] and arq["df"] is not None:
                    ...
                    label_btn = "🔼 Recolher dados" if st.session_state[key_toggle] else f"👁 Ver dados ({arq['linhas']} linhas)"
                    if st.button(label_btn, key=f"btn_toggle_{i}", use_container_width=False):
                        st.session_state[key_toggle] = not st.session_state[key_toggle]
                        st.rerun()
                    if st.session_state[key_toggle]:
                        st.dataframe(arq["df"], use_container_width=True,
                                     height=min(500, max(150, arq["linhas"] * 35 + 50)))
                st.divider()

            # Resumo total
            ok_count  = sum(1 for a in st.session_state.arquivos_carregados if a["ok"])
            total_lin = sum(a["linhas"] for a in st.session_state.arquivos_carregados if a["ok"])
            total_col = sum(a["colunas"] for a in st.session_state.arquivos_carregados if a["ok"])
            st.markdown(
                f'<div class="box-ok">✅ <b>{ok_count} arquivo(s)</b> · '
                f'<b>{total_lin} linhas</b> · <b>{total_col} colunas</b> disponíveis para mapeamento</div>',
                unsafe_allow_html=True)

            # Consolida todos e mostra preview consolidado
            ...
            if dfs_ok:
                ...
                if len(st.session_state.arquivos_carregados) > 1:
                    if "show_consolidado" not in st.session_state:
                        st.session_state.show_consolidado = False
                    lbl_cons = "🔼 Recolher preview consolidado" if st.session_state.show_consolidado else f"👁 Ver preview consolidado ({len(df_cons)} linhas × {df_cons.shape[1]} colunas)"
                    if st.button(lbl_cons, key="btn_toggle_cons", use_container_width=False):
                        st.session_state.show_consolidado = not st.session_state.show_consolidado
                        st.rerun()
                    if st.session_state.show_consolidado:
                        st.dataframe(df_cons, use_container_width=True,
                                     height=min(500, max(150, len(df_cons) * 35 + 50)))
            ...
            st.markdown('<div class="box-info">⬆️ Selecione um arquivo acima e clique em <b>➕ Adicionar</b>.</div>',
                        unsafe_allow_html=True)
