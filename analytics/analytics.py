...
# ── FLUXO ───────────────────────────────────────────
    ...
    CORES_LIGHT=["#2563EB","#059669","#D97706","#DC2626","#7C3AED","#0891B2","#EC4899","#14B8A6"]
    ...
    with t1:
        ...
        if totais_ent:
            fig_ent=go.Figure(go.Bar(x=list(totais_ent.keys()),y=list(totais_ent.values()),
              marker_color=CORES_LIGHT[:len(totais_ent)],opacity=.85))
    ...
    with t2:
        ...
        if totais_sai:
            fig_sai=go.Figure(go.Bar(x=list(totais_sai.keys()),y=list(totais_sai.values()),
              marker_color=CORES_LIGHT[:len(totais_sai)],opacity=.85))
...
# ── CENÁRIOS FP&A ───────────────────────────────────
    ...
    # ── FLUXO PROJETADO
        ...
        st.download_button("📥 Exportar Fluxo de Caixa — Todos os 3 Cenários (Excel)",gerar_excel_fluxo_3_cenarios(),
          file_name="FluxoCaixa_Projetado_3_Cenarios.xlsx",
          mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
          use_container_width=True,key="dl_fc_3cen")
        # KPIs fluxo
        st.divider()
        k=st.columns(4)
        mc(k[0],"Saldo Inicial",fmt(si),"b")
