...
# API KEY — expander recolhido se já tiver chave salva
with st.expander("🔑 Anthropic API Key", expanded=not bool(st.session_state.api_key)):
    ...
    if ak and ak != st.session_state.api_key:
        ...
        # Salva no arquivo config.json — persiste entre sessões
        cfg = carregar_config()
        cfg["anthropic_api_key"] = ak
        salvar_config(cfg)
    if st.session_state.api_key:
        col_ak1, col_ak2 = st.columns([3,1])
        with col_ak1:
            st.markdown('<div class="box-ok">🟢 IA ativa — chave salva, não precisa inserir novamente</div>',unsafe_allow_html=True)
        with col_ak2:
            if st.button("🗑 Remover chave", key="btn_rem_key"):
        ...
        st.markdown('<div class="box-warn">🟡 Insira a API Key — será salva automaticamente para próximas sessões</div>',unsafe_allow_html=True)
    ...
    if tipo == "arquivo":
        ...
        if st.session_state.arquivos_carregados:
            ...
            for i, arq in enumerate(st.session_state.arquivos_carregados):
                ...
                # Preview individual de cada arquivo (todas as linhas)
                if arq["ok"] and arq["df"] is not None:
                    with st.expander(f"👁 Ver dados de {arq['nome']} — {arq['linhas']} linhas"):
